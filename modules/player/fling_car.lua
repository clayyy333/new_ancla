...
local CONFIG = {
	...
	FAR_DISTANCES = {
		4487425, 7554477, 9193601, 11000000,
		12572022, 15000000, 17003482, 21098414,
	},
...
function CarFlingCore:Stop()
	...
	_carFlingMonitorConn = RunService.Heartbeat:Connect(function(dt)
		...
		if car and (not car.Parent or (not isMyCar(car) and not (CarFling.Running and car.Name == "ltp2_car_7"))) then
			if CarFling.Running then CarFling:Stop() end
			CarFling:SetCar(nil)
			if UpdateCarFlingPanel then UpdateCarFlingPanel() end
		end

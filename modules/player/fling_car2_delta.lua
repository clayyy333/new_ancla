...
local CarFling2DeltaCore = {}
CarFling2DeltaCore.__index = CarFling2DeltaCore

function CarFling2DeltaCore.new()
	local self = setmetatable({}, CarFling2DeltaCore)
	self.Running = false
	self.Stopping = false
	self.StopCycle = 0
	self.Connection = nil
...
function CarFling2DeltaCore:SetCar(car)
	self.SelectedCar = car
	return car ~= nil
end

function CarFling2DeltaCore:SetPlayer(player)
	self.SelectedPlayer = player
	return player ~= nil
end

function CarFling2DeltaCore:GetCar()
	return self.SelectedCar
end

function CarFling2DeltaCore:GetPlayer()
	return self.SelectedPlayer
end

function CarFling2DeltaCore:DestroyFlinger()
...
function CarFling2DeltaCore:Disconnect()
...
function CarFling2DeltaCore:CreateFlinger(root)
	self:DestroyFlinger()
	if not root then return end
	local bv = Instance.new("BodyVelocity")
	bv.Name = "VR7CarFling2Deltaer"
	bv.P = CONFIG.P
...
function CarFling2DeltaCore:Stop()
	...
	function CarFling2DeltaCore:GetCarOptions()
		return listMyCars()
	end

	CarFling2Delta = CarFling2DeltaCore.new()
	_carFling2DeltaPlayerRemovingConn = Players.PlayerRemoving:Connect(function(leaving)
		if CarFling2Delta:GetPlayer() == leaving then
	...
	_carFling2DeltaMonitorConn = RunService.Heartbeat:Connect(function(dt)

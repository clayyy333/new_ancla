...
return function(context)
	...
	local card=Instance.new("Frame"); card.Size=UDim2.new(1,-4,0,440); card.BackgroundColor3=currentTheme.secondary; card.ZIndex=7; card.Parent=carControlPanel; Instance.new("UICorner",card).CornerRadius=UDim.new(0,14); RegisterTheme(card,"BackgroundColor3","secondary")
	...
	local autoToggle=button("",282)
	local toggle=button(isES and "Activar control de auto" or "Enable car control",332)
	local status=label("",380); status.Size=UDim2.new(1,0,0,42); status.TextWrapped=true
	...
	UpdateCarControlPanel=function(message)
		...
		autoToggle.Text=(CarControlController:IsAutoEnabled() and "[x] " or "[ ] ")..(isES and "Activación automática" or "Automatic activation")
		autoToggle.BackgroundColor3=CarControlController:IsAutoEnabled() and currentTheme.accent or currentTheme.tertiary
	...
	autoToggle.MouseButton1Click:Connect(function() CarControlController:SetAutoEnabled(not CarControlController:IsAutoEnabled()); UpdateCarControlPanel() end)
	toggle.MouseButton1Click:Connect(function() local ok,err; if CarControlController:IsRunning() then CarControlController:SetAutoEnabled(false); CarControlController:Stop(true); ok=true else ok,err=CarControlController:Start() end; UpdateCarControlPanel(ok and nil or err) end)
